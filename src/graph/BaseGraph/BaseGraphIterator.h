...
public:
    BaseGraphIterator(DirectedSpecifics<T,E>* graph, T* start, bool followIncomingEdges, bool followOutgoingEdges) : graph(graph), followIncomingEdges(followIncomingEdges), followOutgoingEdges(followOutgoingEdges){
        Mutect2Utils::validateArg(graph, "graph cannot be null");
        Mutect2Utils::validateArg(start, "start cannot be null");
    ...
    T* next() {
        ...
        if(visited.find(v) == visited.end()) {
            ...
            if(followIncomingEdges) {
                for(T* vertex : graph->incomingVerticesOf(v))
                    toVisit.push_back(vertex);
            }
            if(followOutgoingEdges) {
                for(T* vertex : graph->outgoingVerticesOf(v))
                    toVisit.push_back(vertex);
            }
        }
        return v;
