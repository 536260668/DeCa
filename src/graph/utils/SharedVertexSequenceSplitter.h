...
private:
    ...
    ArraySet<std::shared_ptr<SeqVertex>> toSplits;
...
public:
    SharedVertexSequenceSplitter(std::shared_ptr<SeqGraph> graph, ArraySet<std::shared_ptr<SeqVertex>> toSplitsArg);
    static std::pair<std::shared_ptr<SeqVertex>, std::shared_ptr<SeqVertex>> commonPrefixAndSuffixOfVertices(ArraySet<std::shared_ptr<SeqVertex>> middleVertices);
