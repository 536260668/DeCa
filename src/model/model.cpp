...
std::vector<std::vector<std::vector<int>>> model::generateData(const std::vector<std::shared_ptr<SAMRecord>> &trimCaseReads,
    ...
    std::vector<std::vector<std::vector<int>>> padOutput = std::vector<std::vector<std::vector<int>>>(5, std::vector<std::vector<int>>(6, std::vector<int>(padReferenceLength, 0)));
        ...
        for(CigarElement readCigarElement : read->getCigarElements())
        {
            ...
            switch (readCigarElement.getOperator())
            {
                case D :
                    ...
                    {
                        for(int i = std::max(vcStart, readStart); i < std::min(readStart + readCigarElement.getLength(), vcEnd); i++)
                        {
    ...
    std::vector<std::vector<std::vector<int>>> result = std::vector<std::vector<std::vector<int>>>(4, std::vector<std::vector<int>>(6, std::vector<int>(31, 0)));
    for(int i = 0; i < 3; i++)
        for(int j = 0; j < 6; j++)
            for(int k = 0; k < 31; k++) {
...
std::vector<std::shared_ptr<SAMRecord>> model::readTrim(std::vector<std::shared_ptr<SAMRecord>> &reads, int start, int end) {
...
bool model::modelRefer(const std::shared_ptr<std::map<std::string, std::vector<std::shared_ptr<SAMRecord>>>> &reads,
    ...
    for (const std::shared_ptr<VariantContext> & vc : withinActiveRegion) {
        ...
        std::vector<std::vector<std::vector<int>>> result = generateData(trimCaseReads, trimNormalReads, allReads, referenceStart, referenceBases.get(), ref_len, vcStart, vcEnd, vc);
        int count2 = 15;
        int index2 = 15;
        while(count2 < 30)
        {
        ...
        for(int i = 0; i < 2; i++) {
            for(int j = 0; j < 31; j++) {
                count[i][j] = result[i+1][0][j] + result[i+1][1][j] + result[i+1][2][j] + result[i+1][3][j] + result[i+1][4][j] + result[i+1][5][j];
            }
        }
        for(int i = 0; i < 6; i++) {
            for(int j = 0; j < 31; j++) {
        ...
        for(int i = 1; i < 3; i++) {
            for(int j = 0; j < 6; j++) {
                for(int k = 0; k < 31; k++) {
                    inputs[i][j][k] = static_cast<float>(result[i][j][k]) / (static_cast<float>(count[i-1][k]) + 0.00000000001f);
                }
            }
        }
        for(int i = 3; i < 5; i++) {
            for(int j = 0; j < 6; j++) {
                for(int k = 0; k < 31; k++) {
                    inputs[i][j][k] = static_cast<float>(result[i-2][j][k]) / (static_cast<float>(result[1][j][k]) + static_cast<float>(result[2][j][k]));
                }
            }
        }
...
void model::Initial() {
    try {
        n_model = torch::jit::load("/Users/bigdreamerxixi/model/trans.pt");
    }
