...
double SomaticLikelihoodsEngine::logEvidence(const std::vector<std::vector<double>> &logLikelihoods,
    ...
    Mutect2Utils::validateArg(numberOfAlleles == priorPseudocounts.size(), "Must have one pseudocount per allele.");
    ...
    if (nonRefIndex > 0) {
        thresholdedPosteriorContribution += log(1- boost::math::ibeta(alleleFractionThreshold,
                                                                      alleleFractionsPosteriorArray->operator[](nonRefIndex), posteriorTotal - alleleFractionsPosteriorArray->operator[](nonRefIndex)));
    }
...
double SomaticLikelihoodsEngine::logDirichletNormalization(std::vector<double> &dirichletParams) {
    ...
    double logDenominator = MathUtils::sum(MathUtils::applyToArray(dirichletParams, [](double a){return lgamma(a);}));
...
double SomaticLikelihoodsEngine::likelihoodsContribution(std::shared_ptr<std::vector<double>> logLikelihoodsForRead,
                                                         std::shared_ptr<std::vector<double>> responsibilities) {
    ...
    for (int n = 0; n < logLikelihoodsForRead->size(); n++) {
        result += (responsibilities->operator[](n) < NEGLIGIBLE_RESPONSIBILITY ? 0 : logLikelihoodsForRead->operator[](n) * responsibilities->operator[](n));
    }
    return result;
}