...
CalledHaplotypes SomaticGenotypeEngine::callMutations(AlleleLikelihoods<SAMRecord, Haplotype> *logReadLikelihoods,
    ...
    {
        logReadLikelihoods->normalizeLikelihoods(NaturalLogUtils::qualToLogErrorProb((uint8_t)MTAC.likelihoodArgs.phredScaledGlobalReadMismappingRate));
    }

    AlleleLikelihoods<Fragment, Haplotype>* logFragmentLikelihoods = logReadLikelihoods->groupEvidence(&SAMRecord::getName, &Fragment::createAndAvoidFailure);
    ...
    for(int loc : startPosKeySet ) {
        ...
        AlleleLikelihoods<SAMRecord, Allele>* logReadAlleleLikelihoods = logReadLikelihoods->marginalize(alleleMapper,
                                                                                                         make_shared<SimpleInterval>(mergedVC->getContig(), mergedVC->getStart(), mergedVC->getEnd())->expandWithinContig(ALLELE_EXTENSION, &header->getSequenceDictionary()));
    ...
    {
        outputCallsWithEventCountAnnotation->emplace_back(VariantContextBuilder(vc).setAttribute(VCFConstants::EVENT_COUNT_IN_HAPLOTYPE_KEY, eventCount)->make());
    }
...
SampleMatrix<Fragment, Allele>* SomaticGenotypeEngine::combinedLikelihoodMatrix(vector<SampleMatrix<Fragment, Allele> *> matrices,
...
SomaticGenotypeEngine::somaticLogOdds(SampleMatrix<Fragment, Allele> *logMatrix) {
    ...
    double logEvidenceWithAllAlleles = logMatrix->evidenceCount() == 0 ? 0.0 : SomaticLikelihoodsEngine::logEvidence(logMatrix->getValuseBySampleIndex(), MTAC.minAF, nonRefIndex);
...
SomaticGenotypeEngine::getNegativeLogPopulationAFAnnotation(vector<shared_ptr<VariantContext>>& germlineResourceVariants,
    ...
    vector<double> populationAlleleFrequencies = getGermlineAltAlleleFrequencies(altAlleles, germlineVC, afOfAllelesNotInGermlineResource);
...
void SomaticGenotypeEngine::addGenotypes(const shared_ptr<AlleleLikelihoods<Fragment, Allele>>& logLikelihoods,
    ...
    for(int n=0; n<numberOfSamples; n++)
    {
        ...
        shared_ptr<vector<double>> alleleFractionsPosterior = logMatrix.evidenceCount() == 0 ? make_shared<vector<double>>(flatPriorPseudocounts) : SomaticLikelihoodsEngine::alleleFractionsPosterior(
        ...
        int * AD = new int[alleleCounts->size()];
...
SomaticGenotypeEngine::getEffectiveCounts(SubsettedLikelihoodMatrix<Fragment, Allele> &logLikelihoodMatrix) {
    ...
    return MathUtils::sumArrayFunction(0, logLikelihoods->operator[](0).size(), [&logLikelihoods](int columnIndex){
        return NaturalLogUtils::normalizeFromLogToLinearSpace(SomaticLikelihoodsEngine::getColumnOfLogLikelihood(*logLikelihoods, columnIndex));
    });
}